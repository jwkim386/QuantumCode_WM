...
ori_watermark_real=imresize(ori_watermark,0.25);
% figure;
...
% GTA_QIR processing
combine_1=[plane_5 plane_6;
           plane_8 plane_7]*1;
% figure;
% imshow(combine_1);

combine_2=[plane_5 plane_6;
           plane_8 plane_7]*1;
% figure;
% imshow(combine_2);
combine_2_180=rot90(combine_2,2);
% figure;
...
combine_4=[plane_8 plane_7;
           plane_5 plane_6]*1;
combine_4_180=rot90(combine_4,0);
% figure;
% imshow(combine_4_180);
...
% image_watermarked_show = imnoise(image_watermarked_show,'gaussian',0.01,zaoyin);
% image_watermarked_show = imnoise(image_watermarked_show,'poisson');
...
real_extrect_lsb1=rot90(plane_extrect_decision_lsb1,2);
...
for i=1:hc
    for j=1:wc
        if i<=h && j<=w
            p_5(i,j)=real_extrect_lsb0(i,j);
            p_55(i,j)=real_extrect_lsb2(i,j);
            p_1(i,j)=real_extrect_lsb1(i,j);
            p_11(i,j)=combine_3_extract(i,j);
        end
         if i<=h && j>w
            p_6(i,j-w)=real_extrect_lsb0(i,j);
            p_66(i,j-w)=real_extrect_lsb2(i,j);
            p_2(i,j-w)=real_extrect_lsb1(i,j);
            p_22(i,j-w)=combine_3_extract(i,j);
        end
        if i>h && j<=w
            p_8(i-h,j)=real_extrect_lsb0(i,j);
             p_77(i-h,j)=real_extrect_lsb2(i,j);
            p_3(i-h,j)=real_extrect_lsb1(i,j);
            p_33(i-h,j)=combine_3_extract(i,j);
        end
      
        if i>h && j>w
            p_7(i-h,j-w)=real_extrect_lsb0(i,j);
            p_88(i-h,j-w)=real_extrect_lsb2(i,j);
            p_4(i-h,j-w)=real_extrect_lsb1(i,j);
...
for i=1:h
    for j=1:w
if p_1(i,j)+p_77(i,j)+p_5(i,j)>1
    p_5(i,j)=1;
elseif p_1(i,j)+p_77(i,j)+p_5(i,j)<=1
    p_5(i,j)=0;
end
    end
end
...
for i=1:h
    for j=1:w
if p_2(i,j)+p_88(i,j)+p_6(i,j)>1
    p_6(i,j)=1;
elseif p_2(i,j)+p_88(i,j)+p_6(i,j)<=1
    p_6(i,j)=0;
end
    end
end
...
for i=1:h
    for j=1:w
if p_4(i,j)+p_66(i,j)+p_7(i,j)>1
    p_7(i,j)=1;
elseif p_4(i,j)+p_66(i,j)+p_7(i,j)<=1
    p_7(i,j)=0;
end
    end
end
...
for i=1:h
    for j=1:w
if p_3(i,j)+p_55(i,j)+p_8(i,j)>1
    p_8(i,j)=1;
elseif p_3(i,j)+p_55(i,j)+p_8(i,j)<=1
    p_8(i,j)=0;
end
    end
end
...
watermark_image_extract_show=rot90(watermark_image_extract_show,3);
 figure;
